...
# 2 image per student or custome number
# import cv2
# import numpy as np
# import pickle
# import os
...
# SAMPLE_PER_STUDENT = 2  # Set to 1 or 2 for a fast 5-minute audit
# LABEL_PATTERN = re.compile(r'^.+ - 2022\d{4}$')

# def run_ultimate_audit():
#     print("🏗️  Initializing Ultimate Audit Engine (Visuals + Analytics)...")
...
#     # Normalize to keep scores between 0.0 and 1.0
...
#     print(f"🔍 Starting Visual Audit. Press 'q' on the image window to stop early.")
...
#             if is_match and not is_verified: color = (0, 165, 255) # Orange for "Right person, low score"
...
#             print(f"{status_icon} Real: {student_folder} | 🤖 Pred: {pred_name} | 🔥 Score: {score:.4f}")
...
#             # Show image
#             cv2.imshow("FUE AMS Audit - Visual Verification", display_img)
#             if cv2.waitKey(1) & 0xFF == ord('q'): break # Short delay so it moves fast
...
#     avg_match = np.mean(match_times)
#     fps = 1000 / (avg_det + avg_match)
